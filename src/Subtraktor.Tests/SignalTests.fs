...
let ``constant returns the same value for any time`` () =
    let s = constant 0.42
    Assert.Equal(0.42, s 0.0)
    Assert.Equal(0.42, s 1.0)
    Assert.Equal(0.42, s 123.456)
...
let ``apply behaves the same as direct function application`` () =
    ...
    Assert.Equal(direct 1.0, viaApply 1.0)
